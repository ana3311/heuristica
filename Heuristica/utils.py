...
class Ramification():
    ...
    def extend(self, items):
        self.A.extend(items) #expandimos
        self.A.sort(key = lambda x: x.path_cost, reverse = True) #ordenamos por más corto
        		   #key = lambda x: x.path_cost -> lambda x: en este caso devuelve el resultado de x.path_cost
class ramificationHeuristic():
    ...
    def extend(self, items): 
        self.A.extend(items) #expandimos
        #ordenamos por más corto (sumándole la heurística primero)
